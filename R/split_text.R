...
as_stg <- function(text, width = NULL, font_size, use = NULL, ...) {
  split_text_grob(
    text = text, width = width, gp = grid::gpar(fontsize = font_size), name = use,
    ...
  )
}
...
arrange_split_txt <- function(..., width = NULL, silent = TRUE) {
  ...
  stg <- tbl %>%
    ...
    # dplyr::mutate(gp = gp) %>% dplyr::mutate(just = just) %>%
    purrr::pmap(as_stg)
  # layout <- tg2layout(stg, width = width) tg_arrange(tg = tg, layout = layout)
}
...
split_string <- function(text, width = grid::unit(1, "npc"), gp = NULL) {
  # base::split() -> stringr::str_split() base::paste() -> stringr::str_c() 'inches' -> 'mm'
  # availwidth -> width width -> stri_width
  if (!is.null(gp)) {
    grid::pushViewport(vp = grid::viewport(gp = gp))
  }
