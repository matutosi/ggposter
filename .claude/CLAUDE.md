...
- 2026-09-02 05:52 (このセッション，x280-home)
  **テストの穴を埋め，リファクタリング3件を入れた** (ユーザ指示の 1・2・3 を順次)．
  - **`layout$align_rows` のテストを新設** (`test-align-rows.R`，23件)．行の揃え・
    短いカードの建て直し (`fit_content = FALSE` で `null` 行になること)・列の
    長さが違う場合・数値 `height` が混ざる場合・1列のときの無効化まで見る．
    **`align_rows` の2巡目を止めると5件が落ちる**ことを確かめた (回帰を捕まえる)．
  - **`poster_title()` のテストを新設** (`test-title.R`，26件)．行の構成と順序・
    `funding` の前の余白行・字の大きさの比・`width` の有無で textGrob と
    textbox_grob が切り替わること・帯の下地・行ごとの書体・ロゴの位置．
  - `with_caption_below()` (図の `caption:`) と `poster_register_font()` の
    テストも追加．**テストは 184 → 246件**．
  - **増えたぶんの重複を落とした** (ユーザの指摘)．`y:` が今も通ることを
    見るテストは `test-grid.R` に同じものがあったので `test-yaml.R` 側を消し，
    「新しい検査がこれを壊さない見張りも兼ねる」と既存側にコメントを残した．
    `poster_size()` の新テストにあった既存と同一の行も落とし，
    `with_caption_below()` の2件 (同じ材料を2度組んでいた) は1つにまとめた．
  - **リファクタリング3件**．(1) `build_column()` と `build_grid_body()` で
    重複していた行高計算を **`resolve_track_mm()`** に括り出した (溢れの警告は
    引数にして，列側は今までどおり黙るようにした)．(2) `plot_area` の `gpar` の
    3重複を **`plot_area_grob()`** 1つに．(3) `build_grid_body()` の検査部分を
    **`validate_grid_boxes()`** へ (150行 → 検査と組み立てが分かれた)．
  - **検証**: 見本4本を組み直し，**リファクタリング前後で画素が完全に一致**
    することを確かめた (`pdftoppm -r 24` のハッシュ比較)．
...
**2026-09-02 に，前回挙げたテストの穴とリファクタリング候補はすべて片づけた**
(上の「現在の状態」を見る)．いま残っているものは無い．

思いついたら足す:

- `card_table()` の CJK 判定は表全体を見る (gridExtra が `base_family` を
  1つしか取らないため)．欧文と和文が混ざる表では，和文側に寄せている．
  セル単位で分けたいなら `tableGrob` を通さない別経路が要る．
- `poster_title()` の `funding_gap` にある `length(row_grobs) > 0` の分岐は，
  `title` が必ず先に積まれるので**常に真**．消してよいが，害も無い．
