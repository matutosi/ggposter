...
```{r}
title <- str_c(rep("Title words. ", 10), collapse="")
caption <- str_c(rep("caption words. ", 10), collapse="")
head(iris) %>%
  booktab_table_grob(title = title, caption = caption, fontsize=26) %>%
  grid::grid.draw()
```
...
font_size_table <- 
  ...
  booktab_table_grob(title="Font size of ggposter", fontsize=26)
base_size_table <- 
  font_size_list %>%
...
head(iris) %>%
  ...
  add_annotation(title="title words", caption="caption words", fontsize=26) %>%
  grid::grid.draw()
```
