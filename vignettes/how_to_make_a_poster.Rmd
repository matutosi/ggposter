...
txt_BODY_33   <- "- Colaboration with ggplot2, cowplot, gridtext or other package may produce more effects."
...
```{r combine_part_1}
grid.newpage()
pushViewport(vp=viewport(width=full, gp=gpar(cex=cex)))
part_1 <- 
  stack_grob(title, 
    appose_grob(
...
jpg_grobs <- 
  ...
  # fix hole width
fix_width_as_is <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], width=half, 
   unify="as_is", space=space)  # same to unify=="width"
fix_width_height <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], width=half, 
   unify="height", space=space)

combined_images <- stack_image_grobs(fix_width_as_is, fix_width_height, width=half, unify="width", space=space)
  # grid::grid.show.layout(combined_images$frame$layout)
grid::grid.draw(combined_images)

  # description
txt_ITEM_40  <- "COMBINE IMAGES"
txt_BODY_41   <- "- appose_image_grobs() and stack_image_grobs()"
txt_BODY_42   <- "- Import images and arrange them in a line (row or col)"
txt_BODY_43   <- "- Can unify height or width of each image (argument: unify)."
txt_BODY_44   <- "- Can fix hole width and height of combined grob (argument: width, height). "
use_images <- arrange_txt(
  txt_ITEM_40, 
  txt_BODY_41, txt_BODY_42, txt_BODY_43, txt_BODY_44, 
  vjust=0.5, widths=half)

grid.newpage()

  # pushViewport(vp=viewport(width=full, gp=gpar(cex=cex)))
image_and_description <- stack_grobs_conv(combined_images, use_images)
  # convertUnit(grobWidth(image_and_description), "mm")
  # convertUnit(grobHeight(image_and_description), "mm")
grid.newpage()
...
image_and_description <- stack_grobs_conv(fix_width_height, use_images)
  # convertUnit(grobWidth(image_and_description), "mm")
  # convertUnit(grobHeight(image_and_description), "mm")
grid.newpage()
grid.draw(image_and_description)

grid::grid.show.layout(fix_width_height$frame$layout)
grid::grid.draw(fix_width_height)

grid.newpage()
grid::grid.show.layout(use_images$frame$layout)
grid::grid.draw(use_images)



  grobs <- list(fix_width_height, use_images)
  n <- length(grobs)
    # layout
  ...
  layout <- grid::grid.layout(nrow=n, ncol=1, widths=widths, heights=heights)
    # frame and place
  combined_grobs <- grid::frameGrob(layout=layout, name=NULL)
...
```{r image_fix_hole_height}
grid.newpage()
combined <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], 
  height=grid::unit(30, "mm"), unify="height", space=space)
grid::grid.show.layout(combined$frame$layout)
grid::grid.draw(combined)

grid.newpage()
combined <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], 
  height=grid::unit(30, "mm"), unify="as_is", space=space)
grid::grid.show.layout(combined$frame$layout)
grid::grid.draw(combined)

grid.newpage()
combined <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], 
  height=grid::unit(25, "mm"), unify="width", space=space)
grid::grid.show.layout(combined$frame$layout)
grid::grid.draw(combined)
```
...
description_1 <- 
  arrange_txt(
    txt_body_31, txt_body_32, txt_body_33, txt_body_34, 
    widths=half)

grid.newpage()
grid.draw(appose_image_grobs(fig, description_1, unify="width", width=half))


grid.draw(colab_ggplot <- appose_grobs(fig, description_1))
