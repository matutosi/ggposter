...
* **`grid:` is the shared interchange form for layout.** ggposter and
  ...
  carry across.
* **The header keys are aliased.** The names the three tools use for the
  same metadata (`author`/`authors`/`poster-authors`,
  `institute`/`institutes`/`affiliations`, `note`/`footer`/`funding`,
  `paper`/`size`, `font-size`/`base_size`) are all accepted here and
  rewritten to ggposter's own; see [poster()] for the full mapping.
