...
fig_pairs <- 
  plot2grob(~pairs(data_c2009), fontsize = 16, width = half) %>%
  grid::grid.draw()
```
...
tbl_fonts <- 
  combine_grobs(tbl_font_size, tlb_base_size, 
    direction="vertical", unify="as_is", space=grid::unit(10, "mm"))

grid::grid.draw(tbl_fonts)
```
...
tbl_list_all <- 
    combine_grobs(tbl_list_1, tbl_list_2, 
                  direction = "horizontal", unify = "as_is", space = grid::unit(10, "mm"))

grid::grid.draw(list_all)
...
```{r apposed_table, opts.label="small"}
...
```{r fixed_apposed_table, opts.label="small"}
