...
>>>>>>> develop
  ...
  layout <- grid::grid.layout(nrow=1, ncol=n, widths=widths + space, heights=max(heights))
    # overwrite setting when unify=="grow"
        ...
        #   }
    # frame and place
  combined_grobs <- frame_place_grobs(grobs, layout, widths, heights, space, by_row=TRUE, gp, name)
  combined_grobs
}
...
#' 
#' @export
frame_place_grobs <- function(grobs, layout, widths, heights, space, by_row=TRUE, gp=grid::gpar(), name=NULL){
...
stack_image_grobs <- function(..., width=NULL, height=NULL, grow=TRUE, unify="width", space=grid::unit(0, "mm"), gp=grid::gpar(), name=NULL){
  ...
  layout <- grid::grid.layout(nrow=n, ncol=1, heights=heights + space, widths=max(widths))
    # overwrite setting when unify=="grow"
        ...
        #   }
    # frame and place
  combined_grobs <- frame_place_grobs(grobs, layout, widths, heights, space, by_row=FALSE, gp, name)
  combined_grobs
}
