...
a bullet list, a figure, a figure with `notes` beside it, a table, a
table with a `caption` below it, and a photo -- and the right column
shows the R code that built the matching card on the left.
...
```{r howto_preview, warning = FALSE, fig.alt = "A tutorial poster titled 'How to Make an Academic Poster', with a left column showing one example of each ggposter card type (bullet list, figure, figure with bullets below, table with bullets to the right, and a photo strip) and a right column showing the R code that built each matching card."}
