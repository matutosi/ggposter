...
appose_grobs_conv <- function(..., space = grid::unit(0, "mm"), gp = grid::gpar(), name = NULL) {
    ...
    heights <- grid::convertUnit(max(grid::unit(rep(1, n), rep("grobheight", n), grobs)), "mm")
    layout <- grid::grid.layout(nrow = 1, ncol = n, widths = widths, heights = heights)
    # frame and place
    combined_grobs <- grid::frameGrob(layout = layout, gp = gp, name = name)
    for (i in seq_along(grobs)) {
...
stack_grobs_conv <- function(..., space = grid::unit(0, "mm"), gp = grid::gpar(), name = NULL) {
    ...
    heights <- grid::convertUnit(grid::unit(rep(1, n), rep("grobheight", n), grobs), "mm") + space
    layout <- grid::grid.layout(nrow = n, ncol = 1, widths = widths, heights = heights)
    # frame and place
    combined_grobs <- grid::frameGrob(layout = layout, gp = gp, name = name)
    for (i in seq_along(grobs)) {
...
#' 
#' @export
appose_image_grobs <- function(..., width = NULL, height = NULL, grow = TRUE, unify = "height", space = grid::unit(0,
    "mm"), gp = grid::gpar(), shrink = 1, name = NULL) {
    grobs <- list(...)
    grobs[sapply(grobs, is.null)] <- NULL  # remove NULL
    if (length(grobs) == 1)
        grobs <- grobs[[1]]
    ...
    if (!is.null(width))
        width <- grid::convertUnit(width, "mm", valueOnly = TRUE)
    if (!is.null(height))
        height <- grid::convertUnit(height, "mm", valueOnly = TRUE)
    ...
    # reverse ratio
    if (unify == "height")
        rev_ratio <- reverse_ratio(heights) else if (unify == "width")
        rev_ratio <- reverse_ratio(widths) else if (unify == "as_is")
        rev_ratio <- 1 else {
        # do nothing if(unify=='grow' ) rev_ratio <- 1 else # do nothing
        rev_ratio <- 1
    ...
    expantion <- 1
    if (!is.null(width)) {
        expantion <- sum(width - grid::convertUnit(space * n, "mm", valueOnly = TRUE))/sum(grid::convertUnit(widths,
            "mm", valueOnly = TRUE))
    ...
    # and place
    combined_grobs <- frame_place_grobs(grobs, layout, widths, heights, space, by_row = TRUE, gp,
        name)
    combined_grobs
}
...
#' 
#' @export
frame_place_grobs <- function(grobs, layout, widths, heights, space, by_row = TRUE, gp = grid::gpar(),
    name = NULL) {
...
reverse_ratio <- function(lengths) {
    grid::convertUnit(max(lengths), "mm", valueOnly = TRUE)/grid::convertUnit(lengths, "mm", valueOnly = TRUE)
}

#' @rdname combine_iamge_grobs
#' @export
stack_image_grobs <- function(..., width = NULL, height = NULL, grow = TRUE, unify = "width", space = grid::unit(0,
    "mm"), gp = grid::gpar(), shrink = 1, name = NULL) {
    grobs <- list(...)
    grobs[sapply(grobs, is.null)] <- NULL  # remove NULL
    n <- length(grobs)
    ...
    expantion <- 1
    if (!is.null(height)) {
        expantion <- sum(height - grid::convertUnit(space * n, "mm", valueOnly = TRUE))/sum(grid::convertUnit(heights,
            "mm", valueOnly = TRUE))
    ...
    # grid::grid.layout(nrow=1, ncol=n, widths=widths + space, heights=max(heights)) } frame
    # and place
    combined_grobs <- frame_place_grobs(grobs, layout, widths, heights, space, by_row = FALSE, gp,
        name)
    combined_grobs
}
