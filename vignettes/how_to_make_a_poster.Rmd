...
## Dpesign policy of ggposter

ggposter supports making a poster for academic conferences. 
...
Nevertheless, ggposter makes it easy to create a poster.
...
Therefore, it may not be possible to use their original forms in ggposter as it is. 
...
In ggposter we do not need to decide the layout before making, 
...
If we do not specify `base_size` in particular, `base_size` of 26 pt will be used.
...
`font_use` express size easy to understand in a use of poster. 
Name of `font_name` is like LaTeX from scriptsize to Huge, and HUGE that is originally added. 
As descrived above, we can choose specify `base_size` or omit it. 
...
NOTE: To make easier to identify each letter, argument `shrink=0.5` is set in the code bellow. 
...
txt_item     <- "This is a item text, too."
txt_BODY     <- "BODY text is larger than body text."
txt_body     <- "This is a body text, too."
...
We can see font size as plot by ggplot2 visualisation as below. 
...
For preparation, argument `shrink` muliply the font size. 
When `shrink=1` is the same size for A0 paper, `shrink=0.25` makes the size for A4 paper. 

If we can chage values of `size`, `base_size`, and others by overwriting `font_size_list`. ???
...
We can bombine grobs with appose_grobs() and stack_grobs(). 
These functions generate a layout of grob arrangement and place grobs. 
In this process, widths and heights of the grobs will be fixed into abstract unit ("mm"). 
...
wide  <- grid::rectGrob(width=unit(2, "cm"), height=unit(1, "cm"), gp=gpar(fill="blue"))
large <- grid::rectGrob(width=unit(2, "cm"), height=unit(2, "cm"), gp=gpar(fill="white"))
apposed <- appose_grobs_conv(small, tall, wide, large, space=unit(2, "mm"))
stacked <- stack_grobs_conv(small, tall, wide, large, space=unit(2, "mm"))
grid::grid.show.layout(apposed$frame$layout)  # show layout
grid::grid.draw(apposed)

grid::grid.newpage()
combined <- appose_grobs_conv(apposed, stacked, space=unit(2, "mm"))
...
At Last, title will be stacked above Objectives and backgrounds on 
the objectives and backgrounds are stacked in a stacked, and then stacked grob are apposed besides `conclusions`. 

We can draw combined grob (part_1). 
In this case, the paper size is A4, so we can check printed small manuscript. 

## Combine image grobs

As is in the case with combining strings shown above, we can combine images. 
...
We can multiple number and various types of image grobs such as jpg and png can be given as arguments. 
...
Since the widhts and heights given by these function might not be correct, 
thus the apposed or stacked table grob will be overwrapped. 
I can not exactly identify the cause, maybe bug of the function `row_heighst()` and `col_widths`. 
Thus, I can not suggest fundamental solution to it. 
...
apposed <- appose_grobs(font_size_table, base_size_table, space=space)
grid::grid.draw(apposed)
```
...
so I recommend users not to use under other situations because of a table grob returned does not fit resied viewpot. 
...
apposed_fixed <- appose_grobs(font_size_table, base_size_table, space=space)
...
grid::grid.newpage()
head(iris) %>%
  booktab_table_grob(title="title words", caption="caption words") %>%
  grid::grid.draw()
...
bacause grobHeight() and grobWidth() become 0 mm !!
