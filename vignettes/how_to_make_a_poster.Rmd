...
```{r title, fig.width=8.3, out.width="100%"}
...
image_and_description <- stack_grobs_conv(image_description, image_combined)
...
grid.newpage()
combined <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], 
  height=grid::unit(30, "mm"), space=unit(10, "mm"))
grid::grid.show.layout(combined$frame$layout)
grid::grid.draw(combined)
sum(combined$frame$layout$widths)
...
slide <- gen_slide(title_1, plot, text, gp_title, gp_text)
grid.draw(slide)
...
txt_author <- "Toshikazu Matsumura (Konan Women's univ.)"
  # txt_affil  <- ""
  # txt_thank  <- ""
txt_kw     <- "keywords: analysis・慧atabese, search species names"
```
