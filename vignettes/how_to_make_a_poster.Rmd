...
```{r}
grobs[[1]]$width <- unit(0.5, "npc")
grobs[[1]]$height <- unit(0.5, "npc")
grobs[[1]]$width <- unit(5, "cm")
grobs[[1]]$height <- unit(5, "cm")
grid.draw(grobs[[1]])
...
width <- grid::convertUnit(width, "mm", valueOnly=TRUE)
  ...
  # grobs <- list(small, tall, wide, large)
...
grobs <- list(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]])
unify <- "height"
...
expantion <- width / width_sum
  ...
  layout <- grid::grid.layout(nrow=1, ncol=n, widths=widths + space, heights=max(heights))
    # frame and place
  combined_grobs <- frame_place_grobs(grobs, widths, heights, layout, name=NULL)
...
ggplot2::ggsave("d:/tmp.pdf", combined_grobs)

grid::convertUnit(widths,  "mm")
grid::convertUnit(heights, "mm")

grid::convertUnit(layout$widths,  "mm")
grid::convertUnit(layout$heights, "mm")
...
combined <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], width=grid::unit(200, "mm"), space=unit(5, "mm"))
grid::grid.show.layout(combined$frame$layout)
grid::grid.draw(combined)

combined <- appose_image_grobs(jpg_grobs[[1]], jpg_grobs[[2]], jpg_grobs[[3]], jpg_grobs[[4]], width=grid::unit(200, "mm"), unify="as_is", space=unit(5, "mm"))
grid::grid.show.layout(combined$frame$layout)
grid::grid.draw(combined)
...
combined <- appose_image_grobs(small, tall, wide, large, width=grid::unit(200, "mm"), space=unit(5, "mm"))
grid::grid.show.layout(combined$frame$layout)
grid::grid.draw(combined)
