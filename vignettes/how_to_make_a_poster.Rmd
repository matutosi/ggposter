...
image_description <- gridtext::textbox_grob(txt, width=half * shrink, gp = gpar(fontsize = 30), use_markdown=FALSE)
image_and_description <- 
  combine_grobs(image_description, fix_width_height, direction = "vertical", unify = "as_is", space=space)
...
head(iris) %>%
  ...
  add_annotation(title="title words", caption="caption words", fontsize=fontsize) %>%
  grid::grid.draw()
```
