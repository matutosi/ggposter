...
cache <- FALSE
prow <- TRUE
knitr::opts_chunk$set(fig.width = fig.width, out.width = out.width, fig.height = fig.height, cache = cache, prow = prow)


  # chunk template
fig.width <- 8.3
out.width <- "100%"
knitr::opts_template$set(part = list(fig.width = fig.width, out.width = out.width, cache = cache, prow = prow))

fig.width <- 4
out.width <- "50%"
fig.height <- 4
out.height <- "50%"
knitr::opts_template$set(small = list(fig.width = fig.width, out.width = out.width, fig.height = fig.height, out.height = out.height, cache = cache, prow = prow))

fig.width <- 4
out.width <- "50%"
fig.height <- 2
out.height <- "50%"
knitr::opts_template$set(wide = list(fig.width = fig.width, out.width = out.width, fig.height = fig.height, out.height = out.height, cache = cache, prow = prow))
```
...
fig_cor <-
  ggplotify::as.ggplot(~corrplot(data_cor)) %>%
  ggplot2grob(fontsize = 16, width = half / 2)

grid::grid.draw(fig_cor)
  # fig_cor <-
  #   ggplotify::as.grob(~corrplot(fig_cor)) %>%
```
...
fig_heatmap <- 
  ggplotify::as.ggplot(~heatmap.2(as.matrix(Boston), scale = "column", trace = "none")) %>%
  ggplot2grob(fontsize = 16, width = half / 2, height = half)
...
The package improve making poster more than using only ggposer, 
