...
half <- grid::unit(half_w, "mm") - grid::unit(20, "mm") * 2
shrink <- 1
...
fig_facet <- 
  ...
  ggplot2grob(fontsize = 16, width = half, height = half / 3)
...
fig_cor <-
  plot2grob(~corrplot(data_cor), fontsize = 16, width = half / 2)

library(gplots)
data(Boston, package = "MASS")
fig_heatmap <- 
  plot2grob(~heatmap.2(as.matrix(Boston), scale = "column", trace = "none"), 
            fontsize = 16, width = half / 2, height = half)
...
fig_pairs <- 
  plot2grob(~pairs(data_c2009), fontsize = 16, width = half / 2)
...
tbls <- 
  combine_grobs(tbl_font_size, tlb_base_size, 
    direction="vertical", unify="as_is", space = space_20) %>%
  add_rect(width = half - space_20)

grob_heights(fig_cor, convert_to="mm")
grob_heights(fig_pairs, convert_to="mm")

grob_heights(tbls, convert_to="mm")
grob_widths(tbls, convert_to="mm")

  # is.grtree <- function (x) inherits(x, "gTree")
  # grobs <- figs
  # if(is.grtree(grob)) grobs <- grobs$children
  # convert_to = "mm"
  ...
  # grob_heights(figs$children, convert_to="mm")
  # children_name <- grid::childNames(grob)
  # grid::getGrob(children[1])
  # path <- grid::gPath(parent_name, children_name[1])
  # grid::getGrob(path)
...
```{r combine_part_1, fig.height = 12}
grid::grid.newpage()
part_1 <- 
  combine_grobs(title, 
    combine_grobs(
      combine_grobs(objectives, backgrounds, direction = "vertical", unify="as_is", space = space_02),
      conclusions, direction = "horizontal", unify="as_is", space = space_02), 
  direction = "vertical", unify="as_is", space = space_02)
grid.draw(part_1)
```
